...
run(){ echo "=== $1 ==="; "$PY" dual_publish.py "$1" "$2" --yt public --ig 2>&1 | grep -E "\[yt\]|\[ig\]|OK |실패|rror|quota|exceeded" | tail -6; echo "--- rc=${PIPESTATUS[0]} ---"; }
run t_megapack.json  money
run g_emergency.json office
echo "=========== B3 DONE ==========="
