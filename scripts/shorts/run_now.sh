...
run(){ echo "=== $1 ($2) ==="; "$PY" dual_publish.py "$1" "$2" --yt public --ig 2>&1 | grep -E "\[yt\]|\[ig\]|OK |실패|rror|quota" | tail -6; echo "--- rc=${PIPESTATUS[0]} ---"; }
run g_pension.json  office
run g_housing.json  office
run x_portfolio.json money
echo "=========== BATCH DONE ==========="
